...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} dear 👋. I am ${global.BotName}, a bot developed by: R to take your WhatsApp usage into next level.
    ...
    ©️ *${global.BotName}* All Rights Reserved: *R*
    `
...
case 'handsomecheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
				if (!text) return replay(`Tag Someone, Example : ${prefix + command} @R`)
...
I am *Miku Nakano*, a bot developed by *RAS*.
